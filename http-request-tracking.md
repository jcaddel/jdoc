...
| 3 | stp.http.request.cache.max | 100,000 | Maximum number of http requests to cache into main memory (per node).  Only has meaning if http request caching is enabled  (`stp.http.request.cache.enabled=true`) |
